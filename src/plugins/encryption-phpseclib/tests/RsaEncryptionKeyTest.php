...
test('encrypt and decrypt round-trip with RSA OAEP', function () {
    ...
    $key = new RsaEncryptionKey('rsa-key', $publicKey, $privateKey);
...
test('encrypt-only mode throws on decrypt', function () {
    ...
    $key = new RsaEncryptionKey('rsa-key', $publicKey);
...
test('wrong key fails decryption', function () {
    ...
    $key1 = new RsaEncryptionKey('k1', $pk1->getPublicKey(), $pk1);
    $key2 = new RsaEncryptionKey('k2', $pk2->getPublicKey(), $pk2);
...
test('encrypt and decrypt with PKCS1 padding', function () {
    ...
    $key = new RsaEncryptionKey('rsa-pkcs1', $publicKey, $privateKey);
...
test('algorithm constant is 4', function () {
    $privateKey = RSA::createKey(2048);
    $key = new RsaEncryptionKey('test', $privateKey->getPublicKey(), $privateKey);

    expect($key->algorithm)->toBe(4);
    expect(RsaEncryptionKey::ALGORITHM)->toBe(4);
});
